...
namespace Intra.Api.Controllers.Api
{
    ...
    public class SupplierController : BaseAuthController
    {
        ...
        {
            var entity = _SupplierRepository.Table.FirstOrDefault(t => t.Id == id);
        ...
        [Route("")]
        [HttpPost]
        public async Task<ResponseModel>Insert([FromBody] SupplierPostDto data)
        {
            if (_SupplierRepository.Table.FirstOrDefault(t => t.Name == data.Name && t.IsArchive==false)!=null)
            {
                return new ResponseModel
                {
                    Status = false,
        ...
        [Route("")]
        [HttpPut]
        public async Task<ResponseModel>Update([FromBody] SupplierDto data)
        {
            if (_SupplierRepository.Table.FirstOrDefault(t => t.Name == data.Name && t.Id != data.Id && !t.IsArchive)!= null)
            {
                    return new ResponseModel
                    {
                        Status = false,
            ...
            var entity = _SupplierRepository.Table.FirstOrDefault(t => t.Id == data.Id);
            if (entity.IsArchive==true)
            {
                return new ResponseModel
                    {
        ...
        [Route("")]
        [HttpDelete]
        public async Task<ResponseModel>Delete([FromBody] SupplierDto data)
        {
            var entity = _SupplierRepository.Table.FirstOrDefault(t => t.Id == data.Id);
            if (entity.IsArchive == true)
            {
        ...
        {
            return new SearchHelper<SupplierDto>().Search(HttpContext, GetQuery(), searchModels, pagesize, pagenumber, sort);
        }
        private IQueryable<SupplierDto>GetQuery()
        {

            return from a in _SupplierRepository.Table
                   where
                       a.IsArchive == false
